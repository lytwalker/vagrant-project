...
if(!empty($_GET['ifa'])){
	...
	$title = "Pending activation - base stations";
                                ...
                                <?php while($row = $list->fetch_assoc()) { ?>
                                    <tr>
                                        ...
                                        <?php
                                            ...
                                            if($accessLevel > 1 ){
                                            ?> 
                                            <a href="edit.php?baseid=<?= $row['Id'] ?>" data-name="<?= $row['Name'] ?>" title="Edit <?= $row['Name'] ?>" data-toggle="tooltip"><i class="fa fa-pencil"></i></a> <a href="kpis.php?baseid=<?= $row['Id'] ?>" data-name="<?= $row['Name'] ?>" title="KPIs for <?= $row['Name'] ?>" data-toggle="tooltip"><i class="fa fa-signal"></i></a> <a href="polygons.php?baseid=<?= $row['Id'] ?>" data-name="<?= $row['Name'] ?>" title="Polygons for <?= $row['Name'] ?>" data-toggle="tooltip"><i class="fa fa-map"></i></a> <a class="delete" href="delete_base.php?baseid=<?= $row['Id'] ?>" data-name="<?= $row['Name'] ?>" title="Delete <?= $row['Name'] ?>" data-toggle="tooltip"><i class="fa fa-trash"></i></a>
                                        <?php } ?>
                                        </td>
                                    </tr>
                                    <?php } ?>
