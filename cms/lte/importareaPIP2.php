...
class importLTEAreas {
    ...
    function getPolygonData($_altitudeMode, $_coordinates, $_count) {
            ...
            if(!empty($acoordinate)){
                ...
                if($shortlongitude != 0 && $shortlatitude != 0) {                    
                    ...
                    if(empty($this->approvedPolygons)){
                        echo "approvedPolygons is empty so add".$shortlongitude." ".$shortlatitude." to currentPolygon.<br>";
                        array_push($this->currentPolygon, $shortlongitude." ".$shortlatitude);
                    } else {
                        foreach($this->approvedPolygons as $key => $approvedPolygon) {
                            $point = $shortlongitude." ".$shortlatitude;
                            //foreach($this->currentPolygon as $key => $point) {
                                ...
                                if($outside) {
                                    ...
                                    array_push($this->currentPolygon, $point);
        ...
        * ***/        
        if(empty($this->approvedPolygons)){
            echo "<br>approvedPolygons is empty so add currentPolygon to approvedPolygons.<br>-----<br>";
